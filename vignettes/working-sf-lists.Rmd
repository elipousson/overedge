...
nc_list_name <-
  as_sf_list(nc, col = "NAME")

str(nc_list_name)
```

If you use a grouping column, the list names are taken from the value of the grouping column. Alternatively, a character string with the same length as the list can be provided to the nm parameter:
...
`get_location` also supports the creation of sf lists using the name_col, id_col, or locationname_col (used by `get_esri_data`) as a grouping column.
